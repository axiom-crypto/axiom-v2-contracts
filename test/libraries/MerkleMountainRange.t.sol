...
contract MerkleMountainRangeTest is Test {
    ...
    function test_getCompleteLeaves() public {
        MerkleMountainRange.MMR memory mmr1;
        mmr1.peaksLength = 4;
        mmr1.peaks[0] = keccak256(abi.encodePacked(uint256(0)));
        mmr1.peaks[1] = keccak256(abi.encodePacked(uint256(1)));
        mmr1.peaks[2] = keccak256(abi.encodePacked(uint256(2)));
        mmr1.peaks[3] = keccak256(abi.encodePacked(uint256(3)));

        MerkleMountainRange.MMR memory mmr2 = mmr1.getCompleteLeaves(2);
        assert(mmr2.peaksLength == 2); // "peaksLength do not match");
        for (uint256 i = 0; i < 2; i++) {
            assert(mmr2.peaks[i] == mmr1.peaks[i + 2]); // "peaks do not match");
        }
    }

    function test_getZeroPaddedMerkleRoot() public {
        MerkleMountainRange.MMR memory mmr1;
        mmr1.peaksLength = 4;
        mmr1.peaks[0] = keccak256(abi.encodePacked(uint256(0)));
        mmr1.peaks[1] = keccak256(abi.encodePacked(uint256(1)));
        ...
        bytes32 root = mmr1.getZeroPaddedMerkleRoot(2);
        bytes32 answer =
            keccak256(abi.encodePacked(mmr1.peaks[1], keccak256(abi.encodePacked(mmr1.peaks[0], bytes32(0)))));
        assert(root == answer); // "root do not match");
    }
