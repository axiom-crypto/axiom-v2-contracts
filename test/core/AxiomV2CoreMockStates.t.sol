...
contract AxiomV2CoreMockStatesTest is AxiomTestBase {
    ...
    function test_backfill() public {
        file = vm.readFile("test/data/core/mock/backfillTest_1_3867456_3000.json");
        uint32 forkBlock = abi.decode(vm.parseJson(file, ".recoveryBlockNumber"), (uint32));

        _forkAndDeploy("sepolia", forkBlock);
        _runBackfillTest("test/data/core/mock/backfillTest_1_3867456_3000.json");
    }
}
