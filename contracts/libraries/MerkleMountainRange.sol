...
library MerkleMountainRange {
    ...
    function fromPeaks(bytes32[] memory peaks) internal pure returns (MMR memory out) {
        return fromPeaks(peaks, 0, uint32(peaks.length));
    }
    ...
    /// @return out The MMR in memory
    function fromPeaks(bytes32[] memory peaks, uint32 start, uint32 length) internal pure returns (MMR memory out) {
        out.peaksLength = length;
        for (uint32 idx; idx < length;) {
    ...
    function clone(MMR storage self) internal view returns (MMR memory out) {
        ...
        for (uint32 i; i < out.peaksLength;) {
    ...
    function persistFrom(MMR storage self, MMR memory newMMR, uint32 peaksChanged) internal {
        self.peaksLength = newMMR.peaksLength;

        for (uint32 i; i < peaksChanged;) {
    ...
    function commit(MMR memory self) internal pure returns (bytes32) {
        ...
        for (uint32 i; i < self.peaksLength;) {
    ...
    function appendLeaf(MMR memory self, bytes32 leaf) internal pure returns (uint32 peaksChanged) {
        bytes32 newPeak = leaf;
        uint32 i;

        for (; i < self.peaksLength && self.peaks[i] != bytes32(0);) {
    ...
    function appendLeaves(MMR memory self, bytes32[] memory leaves) internal pure returns (uint32 peaksChanged) {
        ...
        while (toAdd != 0) {
            ...
            uint32 nextAdd = (toAdd + shift) >> 1;

            for (uint32 j; j < nextAdd;) {
                bytes32 left;
                bytes32 right;
                ...
                leaves[j] = Hash.keccak(left, right);
                unchecked {
                    ++j;
                }
            }
     ...
     */
    function getZeroPaddedMerkleRoot(MMR memory self, uint32 paddingDepth) internal pure returns (bytes32) {
        bytes32 root;
        bool started;

        for (uint32 peakIdx; peakIdx < paddingDepth;) {
     ...
     */
    function getComplementMerkleRoot(MMR memory self, uint32 paddingDepth, bytes32[] memory mmrComplement)
        internal
        pure
        returns (bytes32)
    {
