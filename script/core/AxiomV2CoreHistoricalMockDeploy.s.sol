...
contract AxiomV2CoreHistoricalMockDeploy is AxiomDeployBase {
    function run(uint64 sourceChainId, bool isProd, uint256 saltNum) external {
        ...
        console.log("Deploying AxiomV2CoreHistoricalMock");
        ...
        vm.startBroadcast();
        address verifier = _deployCoreVerifier(sourceChainId);
        address coreHistoricalMock = _deployCoreHistoricalMock(verifier, timelock, guardian, unfreeze, coreProver, salt);
        vm.stopBroadcast();

        console.log("AxiomV2CoreVerifier:", verifier);
        console.log("AxiomV2CoreHistoricalMock:", coreHistoricalMock);
    }
}
